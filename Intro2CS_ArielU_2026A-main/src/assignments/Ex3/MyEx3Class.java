...
public class MyEx3Class extends Ex3Algo {
    ...
    public int Move(PacmanGame game) {
        ...
        Index2D pac = (Index2D) me;
        if (currentTarget == null || gameMap[currentTarget.getX()][currentTarget.getY()] != PINK) {
            currentTarget = findClosestFood(map, gameMap, (Index2D) me);
        }
        for (int dir : new int[]{Game.UP, Game.RIGHT, Game.DOWN, Game.LEFT}) {
            ...
            if (currentTarget != null) {
                double d = depthMap.getPixel(currentTarget.getX(), currentTarget.getY());
                if (d != -1) score += 10.0 / (1.0 + d);
            }
            if (score > bestScore) {
                bestScore = score;
                bestDir = dir;
